...
public class NewOrderDialog extends Dialog {
    ...
    ObservableList orderData;
    
    public NewOrderDialog(ObservableList orderData){
   ...
   fillTheComboProduct();
   chooseCustomer.setOnAction(Action ->{
       int index = comboCustomer.getSelectionModel().getSelectedIndex();
       showCustomer(index);
   ...
   saveOrderButton.setOnAction(action -> {createOrder();});
    ...
    Customer c = customerManager.getAll().get(index);
    idCustomerField.setText(c.getId()+"");
    fNameCustomerField.setText(c.getFirstName());
    lNameCustomerField.setText(c.getLastName());
    phoneCustomerField.setText(c.getPhone());
    }    
   ...
   if(totalTva == 0){
   alert.show("Oops!", "Please check the total button", Alert.AlertType.ERROR);
   return;
   }
   alert.show("", "Are you sure to create this order ?", Alert.AlertType.CONFIRMATION);
   if(alert.getAction().get() == ButtonType.OK){
   orderManager.insert(new Order(0, customerManager.getAll().get(comboCustomer.getSelectionModel().getSelectedIndex()).getId(), "description", totalTva, orderDate));
   //System.out.println(orderManager.getLastId());
   for(int i = 0; i < listLineOrder.size(); i++){
   ...
   showOrderInTheTable();
   alert.show("done!", "Order created !", Alert.AlertType.INFORMATION);
   clearDate();
   this.hide();
   }
    ...
    private void showOrderInTheTable(){
    orderData.add(orderManager.getAll().get(orderManager.getAll().size() - 1));
    }
    
    private void clearDate(){
    listLineOrder.clear();
    dataLineOrder.clear();
    comboCustomer.getSelectionModel().select(-1);
    idCustomerField.clear();
    fNameCustomerField.clear();
    lNameCustomerField.clear();
    phoneCustomerField.clear();
